...
const ContactSection: React.FC = () => {
    ...
    const email = 'validsstorage@gmail.com';
    ...
    const handleCopyClick = () => {
        if (navigator.clipboard) {
            navigator.clipboard
                .writeText(email)
                .then(() => {
                    addPopup(t('sections.contact.copySuccess'), 'success');
                })
                .catch(() => {
                    addPopup(t('sections.contact.copyError'), 'error');
                });
        } else {
            addPopup(t('sections.contact.copyError'), 'error');
        }
    ...
    return (
        <div className={styles.container} id="contact" ref={ref}>
            <div className={styles.sideWrapper}>
                <div className={`${styles.lSide} ${isVisible ? styles.animate : ''}`}>
                    <h1 className={styles.title}>{t('sections.contact.title')}</h1>
                    <h2 className={styles.subtitle}>{t('sections.contact.subtitle')}</h2>
                    ...
                    <div className={styles.contactInfo}>
                        <p>
                            ...
                            <button
                                ...
                                className={`${styles.copyButton} ${styles.iconWithTooltipCopy}`}
                                aria-label="Copy Email"
                                type="button"
                            >
                                <Copy />
                                <span className={styles.tooltipCopy}>{t('sections.contact.copy')}</span>
                            </button>
                        </p>
                    </div>
                    ...
                    <div className={styles.socialIcons}>
                        <a
                            href="https://www.facebook.com/patrak.blue/"
                            target="_blank"
                        ...
                        <a
                            href="https://www.instagram.com/patrakblue/"
                            target="_blank"
                            rel="noopener noreferrer"
                        ...
                        <a
                            href="https://discord.com/users/465398318688763915"
                            target="_blank"
                            rel="noopener noreferrer"
                            aria-label="Github Profile"
                            className={styles.iconWithTooltip}
                ...
                <div className={`${styles.rSide} ${isVisible ? styles.animate : ''}`}>
                    <ContactForm />
                </div>
            </div>
            <StarsCanvas />
